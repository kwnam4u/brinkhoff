 ...
 * Uses the Oracle database reporter and reads the network from the database, where it is stored using Oracle Spatial.
 * See the additional documentation for the necessary table definitions and the supported properties in the property file.
 * Requires classes12.zip and sdoapi.zip in the classpath.
 *
 * @version	1.01	03.09.2003	constructing of DrawableObject-objects removed, makeAbsolute called
...
/**
 * Creates a node. If a node with the given coordinate exists, this node will be returned.
 * Otherwise, a new node will be constructed.
 * @return the node
 * @param id identifier
...
protected void startLoadingThread () {
	try {
		...
		// query extent
		SpatialResultSet extent = new SpatialResultSet(query.executeQuery(
			"SELECT SDO_TUNE.EXTENT_OF('"+tableName+"','"+geoAttrName+"') FROM DUAL"));
		extent.next();
		...
		while (feature.next()) {
			...
			java.awt.Polygon pol = GeometryConversion.toAwtPolyline(feature.getGeometry(geoAttrName));
			// construct nodes and edges
			Node n1 = computeNode(++id,pol.xpoints[0],pol.ypoints[0],nodes);
			for (int i=1; i<pol.npoints; i++) {
				Node n2 = computeNode(++id,pol.xpoints[i],pol.ypoints[i],nodes);
