...
/**
 * Constructor.
 * @param  applet  the ShowMap applet
 * @param  url  one or more URLs to read
 * @param  objNum  number of objects read before
 */
 ...
 * After the reading, the state of the applet is set to ShowMap.COMPLETE and the map is drawn.
 */
public void run () {
	...
	if (applet != null)  {
		if (applet.getState() < ShowMap.ACTIVE)
			applet.setViewToPrefinedValue();
		applet.setState (ShowMap.COMPLETE);
		applet.checkViewPoint();
